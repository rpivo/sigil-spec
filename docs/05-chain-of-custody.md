...
Earlier drafts of this repository assumed a single signer attesting to a finished recording.
The intended model is broader: a **chain of custody**, where each stage in the production
...
3. The interface passes audio to a DAW, which verifies the interface as the source.
4. The DAW exports a mixdown carrying an attestation of the session's sources.
...
**Attestation begins at the analog-to-digital boundary.** The A/D converter is the first
...
manufacture and a secure element to hold it, which places a real cost and key management
...
history exceeds any plausible carrier capacity. See R12 in
[04-requirements.md](04-requirements.md).

This granularity aligns closely with the DDEX AI disclosure categories, which suggests
...
| **1. Attested capture** | Interface hardware with a secure element and provisioned keys. | Samples demonstrably entered through an identified device. |
...
**C2PA already defines assurance levels** expressing how strongly an attestation is
...
- How this survives stem delivery, where a release is assembled from separately exported
  files.
